...
en plus de la sortie standard. Utile pour comprendre après coup ce qui
s'est passé pendant une compétition (imports, validations, erreurs...)
sans dépendre d'un terminal resté ouvert. Port fidèle de l'équivalent
FletchTime (voir fletchtime/logging_setup.py) -- même mécanisme, adapté
au nom du paquet.
"""
...
# compétitions de suite avant qu'un fichier ne soit écrasé, sans grossir
...
def configure_logging(
    ...
    configuré (utile si un test appelle ceci plusieurs fois dans le même
    processus). Retourne le chemin du fichier de journal, pour affichage
    ...
    fiable) même si le terminal reste silencieux (WARNING par défaut).
    Voir ``fletchscore.__main__``, options ``-v``/``--verbose`` (INFO sur
